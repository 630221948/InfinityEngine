...
var GridMap = (function () {
    function GridMap(mapNodeArray) {
        ...
        for (var i = 0; i < mapNodeArray.length; i++) {
            ...
            if (mapNodeArray[i].img.url == "../resources/Road.png") {
                var node = new MapNode(x, y, true);
                this.config.push(node);
            }
            else {
                var node = new MapNode(x, y, false);
                this.config.push(node);
            }
        }
        // this.config = [
    ...
    GridMap.prototype.getNodeCounts = function () {
        return this.nodeCounts;
    };
    GridMap.prototype.getNode = function (x, y) {
        var result = this.config[y * this.row + x];
        return result;
    };
    return GridMap;
}());
